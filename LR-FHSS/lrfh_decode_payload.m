function [decoded_pay,match,min_cost]=lrfh_decode_payload(deint_payload,CR_de, LRF_cfg)
    if 1
        local_mul = 128/LRF_cfg.demod_soft_val_cap - 1; % NOTE: -1 to avoid overflow
        deint_payload_quant = round(local_mul*deint_payload) + 128;
        
        tbdepth = 66;
        if CR_de == 3
            if length(deint_payload)/3 > tbdepth
                decoded_pay = vitdec(deint_payload_quant,LRF_cfg.myTrellis,tbdepth,'term','soft',8);
            else
                decoded_pay = zeros(1,floor(length(deint_payload)/3)); decoded_pay(end-24:end) = 1;
            end
        elseif CR_de == 1
            puncpat = [1 1 0 0 1 0];
       ...
       [decoded_pay,match,min_cost] = vit_decode_payload(0,deint_payload,LRF_cfg.myTrellis,CR_de);
    end
end
